...
static int8_t yc_uart_open	(void);				// Initializes UART peripheral
static int8_t yc_uart_close	(void);				// Finishes UART peripheral
static int8_t yc_uart_write	(char*, uint16_t);	// Write the content in TX buffer to transmit
...
/**
  ...
  * @pre-cond.  : yc_uart_open() must be called first
...
/**
  ...
  */
static int8_t yc_uart_open(void)
{
    /* Configuring UART1 ******************************************************/   
...
/**
  ...
  * @pre-cond.  : yc_uart_open() already called
  * @post-cond. : UART disabled and stopped
  * @parameters : void
  * @retval     : int8_t -> (value == 0) = OK; (value < 0) = ERROR
  */
static int8_t yc_uart_close(void)
{
...
/**
  ...
  * @retval     : int8_t 	-> (value == 0) = OK; (value < 0) = ERROR
  */
static int8_t yc_uart_write(char *pvVal, uint16_t uiSize)
{
	// Check if UART was enabled
	if(UART.FlagEnable)
	{
