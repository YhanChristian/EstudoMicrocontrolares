...
void startSDCard() {
     unsigned short i;
    //inicia a comunicaçao com cartão SD --Inicialização do SPI com divisor de 64 (CLK/64)
...
void writeFileSDCard(unsigned short current[4], int voltage[4], unsigned int activePower[4]) {
     ...
     // Cria arquivo caso não exista - olhar help Mikro C para funções SDCard
     Mmc_Fat_Assign(&filename, 0xA0);

     // -- Escrita arquivo CSV e separação por ";"
     ...
     // -- Definições e criação do arquivo, gravação e fechar conexão.
