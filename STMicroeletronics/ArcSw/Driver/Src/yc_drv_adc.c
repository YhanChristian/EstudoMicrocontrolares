...
static int8_t 	yc_adc_open (void);	 // Opens ADC peripheral
static int8_t   yc_adc_close(void); 	// Closes ADC peripheral
static int32_t  yc_adc_read (e_adc_t);	// Read ADC channel value
...
/**
  ...
  * @pre-cond.  : yc_adc_open() must be called first
...
/**
  ...
  */
static int8_t yc_adc_open(void)
{    
    ...
    /** Configure all regular channels ****************************************/
...
/**
  ...
  * @retval     : int8_t -> (value == 0) = OK; (value < 0) = ERROR
  */
static int8_t yc_adc_close(void)
{
...
/**
  ...
  * @pre-cond.  : yc_adc_open() already called
  * @post-cond. : ADC channel value read
  * @parameters : eChannel
  * @retval     : int8_t -> (value >= 0) = OK; (value < 0) = ERROR
  */
static int32_t yc_adc_read(e_adc_t eChannel)
{    
    // Check if ADC was enabled
    if(AD.FlagEnable)
    {			
