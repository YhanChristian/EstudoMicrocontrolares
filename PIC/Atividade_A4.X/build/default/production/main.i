...
float fGetVoltage(int iValue) {
    return (5 * (float)iValue) / (float)1023;
}

float fGetPercentDuty(int iValue) {
    return (100 * (float)iValue) / (float)1023;
}
...
void MsgInicialLcd() {
    ...
    _delay((unsigned long)((2000)*(20E6/4000.0)));
...
void SystemControl() {
    ...
    if (gstTimer1Seg.fTimerOverflow) {
        ...
        iValue = iAnalogRead(0);


        iSt = iPWMSetDuty(2, iValue);
        if (iSt) {
        ...
        if (iValue > 0) {
            iResult = (iValue / 128) + 1;
        }
        setLedsOn((uint8_t) iResult);


        changeBuzzerState((iResult == 8) ?!readBuzzerState() : 0);
