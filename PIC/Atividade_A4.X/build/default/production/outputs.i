...
void changeBuzzerState(_Bool fState);
_Bool readBuzzerState();
# 13 "outputs.c" 2
...
void pinMode(volatile unsigned char *pcTris, uint8_t uiPin, _Bool fMode) {
    if (!fMode) {
...
void digitalWrite(volatile unsigned char *pcLat, uint8_t uiPin, _Bool fState) {
    if (fState) {
...
_Bool digitalRead(volatile unsigned char *pcPort, uint8_t uiPin) {
...
void configOutputs() {
    ...
    for (i = GPIO_LED_0; i < MAX_LEDS; i++) {
        pinMode(&TRISB, (uint8_t) i, 0);
        digitalWrite(&LATB, (uint8_t) i, 0);
    }

    pinMode(&TRISC, 1, 0);
    digitalWrite(&LATC, 1, 0);
}
...
void changeBuzzerState(_Bool fState) {
    digitalWrite(&LATC, 1, fState);
}

_Bool readBuzzerState() {
    return digitalRead(&PORTC, 1);
}
